...
#  include <windows.h>
   ...
   static inline double elapsed_sec(timespec_t *a, timespec_t *b){ return (double)(b->QuadPart - a->QuadPart)/(double)_qpf.QuadPart; }
#else
#  include <time.h>
   typedef struct timespec timespec_t;
   static inline void timer_init(){}
   static inline void clock_now(timespec_t *t){ clock_gettime(CLOCK_MONOTONIC, t); }
   static inline double elapsed_sec(timespec_t *a, timespec_t *b){ return (b->tv_sec - a->tv_sec) + (b->tv_nsec - a->tv_nsec)*1e-9; }
#endif
...
__global__ void ac_kernel(
    ...
    int                        num_states,
    int                        max_pat_len,
    unsigned long long * __restrict__ match_count,   /* total matches (atomic)      */
    int                        chunk_sz)
{
